...
#this wraps the dataset so you can actually tell which images are already broken(not_suc), vs
#which images still need attacking (need_atk).
class ImageSet(Dataset):
    def __init__(self, df, transformer,datasets):
        # df is the dataframe with image paths and labels.
        #transformer deals with preprocessing and tensor conversion.
        #datasets tells you which dataset you are going to use: (mnist, cifar, imagenet) 
        self.df = df
...
# margin loss
#this is the measure the attack is done with respect to.
#it satisfies the core adversarial objective of having another class have a class
#score higher than the true class. 
def margin_loss(logits,y,i=0,MT=True):
    #here the logits are the model outputs, y is the true class label. 
    bs = len(y)
    Y = y.view(-1,1)
    logit_org = logits.gather(1,Y)
    #this removes the true class by subtracting a large number. 
    T = torch.eye(logits.shape[1], device=logits.device)[y.to(logits.device)] * 999999
    TA = (logits-T)
    #this picks a competing class. 
    ...
    if MT:
        ...
        #this tries to maximize the difference(flip classification) 
...
#this is the heart of the attack. it has two phases, ADI exploring directions, and attack phase. 
def AAA_white_box(model, X, X_adv, y, epsilon=0.031, step_num=20, ADI_step_num=8,
    ...
    np_atk_filed_index = np.ones(len(y)).astype(bool)
    ...
    for i in range(ADI_step_num + step_num):
        #Here ADI selects only images that are still correctly classified. 
        ...
        tbn+=X_adv_f.shape[0]
        if i<ADI_step_num:
            #this is basically the PGD style update with the cosine adaptive stepsize. 
            if Lnorm =="Linf":
        ...
        if Lnorm == 'Linf':
            #this basically keeps the perturbation within the epsilon e ball. 
...
def Adaptive_Auto_white_box_attack(model, device, eps, is_random, batch_size, average_num, model_name, data_set="cifar10", Lnorm='Linf', progress_lam=0.0, score_strategy=None, top_fraction=None):
    ####  1.Loading datasets  ####
    if data_set=="mnist":
        num_class=10
    ...
    not_suc_index = np.ones(len(all_atk_labs)).astype(bool)
    ...
    prev_max_loss = max_loss.copy()
    full_preselect_score = None
    logits_num = num_class
    sorted_logits_index = np.zeros((ori_need_atk_num,logits_num),dtype=int)
    natural_acc = 0
    begin_adv_acc = 0
    No_epoch = -2
    total_bn = 0
    ...
    for i, test_data in enumerate(data_loader):
        ...
        ######This is one of the key parts of my experiment that I implemented. Here we have the four kinds of loss functions
        ######logloss, loss, confidence and Margin. 
        if score_strategy is not None:
            ce = torch.nn.functional.cross_entropy(out_clean, y, reduction="none")
            probs = torch.softmax(out_clean, dim=1)
            true_conf = probs[torch.arange(len(y), device=out_clean.device), y]

            true_logits = out_clean[torch.arange(len(y), device=out_clean.device), y]
            masked_logits = out_clean.clone()
            masked_logits[torch.arange(len(y), device=out_clean.device), y] = -1e9
            best_other_logits = masked_logits.max(dim=1).values
            margin = best_other_logits - true_logits

            if score_strategy == "loss":
                raw_score = ce
            elif score_strategy == "log_loss":
                raw_score = torch.log1p(ce)
            elif score_strategy == "confidence":
                raw_score = 1.0 - true_conf
            elif score_strategy == "margin":
                raw_score = margin
            else:
                raise ValueError(f"Unknown score_strategy: {score_strategy}")

            if full_preselect_score is None:
                full_preselect_score = np.zeros(ori_need_atk_num, dtype=float)
            full_preselect_score[bstart:bend] = raw_score.detach().cpu().numpy()

        
    max_loss[~not_suc_index]= np.ones((~not_suc_index).sum())
    if score_strategy is not None and full_preselect_score is not None:
        full_preselect_score = (full_preselect_score - full_preselect_score.mean()) / (full_preselect_score.std() + 1e-8)
    ...
    for out_re in trange(out_restart_num):
        ...
        for r in range(restart_num):
            ...
            ######Here is a little bit more of the code I added. It specifies that if the score strategy is not none, then we use the score strategy
            ######Which I suggested, namely, Score = current loss + 𝜆 (current loss – previous loss).
            if score_strategy is not None:
                score_pool = full_preselect_score.copy()
                score_pool[~not_suc_index] = -1e9
                k = min(need_atk_num_place1, int(not_suc_index.sum()))
                max_K = np.partition(score_pool, -k)[-k]
                loss_need_atk_index = (score_pool >= max_K) & not_suc_index
            else:
                progress_score = max_loss + progress_lam * (max_loss - prev_max_loss)
                max_K = np.partition(progress_score, -need_atk_num_place1)[-need_atk_num_place1]
                loss_need_atk_index = (progress_score >= max_K) & (max_loss < 0)
            ...
            now_prev_max_loss = now_max_loss.copy()
            ...
            prev_max_loss[not_suc_need_atk_index] = now_prev_max_loss
...
#Here we have the possibility of using the 50 different models used in the paper. I was only able to access the file available in their git repository.
#the key things this does is loads the specific model, sets the value for epsilon and the dataset, and runs the actual code. 
def main(flag, model_name, ep=8./255, random=True, batch_size=128, average_number=100, progress_lam=0.0, score_strategy=None, top_fraction=None):
    ...
    model.to("cuda")
    model.eval()
    Adaptive_Auto_white_box_attack(model, device, ep, random, batch_size, average_number, model_name, data_set=data_set, Lnorm=Lnorm, progress_lam=progress_lam, score_strategy=score_strategy, top_fraction=top_fraction)
